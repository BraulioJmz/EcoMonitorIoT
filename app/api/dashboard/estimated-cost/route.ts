...
export async function GET(request: NextRequest) {
  try {
    ...
    // Obtener la tarifa actual (usar tarifa_pico por defecto)
    const tariff = await prisma.tarifas.findFirst({
      orderBy: {
        updated_at: 'desc'
      }
    })

    const tariffRate = tariff?.tarifa_pico || 0.15 // Default $0.15 por kWh
    ...
    if (calculationMode === 'total_consumption' && startDateParam && endDateParam) {
      ...
      console.log('🔍 Calculando costo desde daily_energy_totals:', { start, end })
      ...
      const estimatedCost = netKwh * tariffRate
      
      console.log('✅ Costo desde daily_energy_totals:', { totalImported, totalExported, netKwh, estimatedCost })
    ...
    // (cuando hay machineId pero NO hay machineIdsParam ni calculationMode personalizado)
    if (machineId && !machineIdsParam) {
      ...
      if (startDateParam && endDateParam) {
        ...
        if (machineId === "7") {
          ...
          const totalImported = (rows && rows.length > 0 && rows[0].kwh_imported) ? Number(rows[0].kwh_imported) : 0
          const estimatedCost = totalImported * tariffRate
    ...
    if (startDateParam && endDateParam) {
      ...
      // Calcular costo estimado: (imported - exported) * tarifa
      const netKwh = totalImported - totalExported
      const estimatedCost = netKwh * tariffRate
    ...
    if (machineId) {
      ...
      if (machine) {
        ...
        if (rows && rows.length > 0) {
          ...
          const estimatedCost = accumulated_kwh * tariffRate
          return NextResponse.json({
            estimatedCost: estimatedCost,
    ...
    return NextResponse.json(
      { error: 'Error al obtener costo estimado' },
      { status: 500 }
    )
  }
}
